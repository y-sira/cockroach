...
func (mb *mutationBuilder) buildFKChecksForUpdate() {
	...
	for i, n := 0, mb.tab.InboundForeignKeyCount(); i < n; i++ {
		...
		h := &mb.fkCheckHelper
		if !h.initWithInboundFK(mb, i) {
			// FK relation can safely be ignored.
			continue
		}
...
func (h *fkCheckHelper) initWithOutboundFK(mb *mutationBuilder, fkOrdinal int) {
	*h = fkCheckHelper{
		mb:         mb,
